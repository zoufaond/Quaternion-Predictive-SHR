function das3_polynomials(osimfile,mydir,motion,GH_seq,gen_polyvalues,musclepolyfile) %GHpolyfile,
...
if strcmp(gen_polyvalues,'gen_polyvalues')
    ...
    % for imus = 60:62
        ...
        alljntsQ = [];
        alljntsQA = [];
        jnts2change_seq = {'YZX','YZX','YZX',GH_seq};
        num_jnts2change = length(jnts2change_seq);
        numdata = size(angles,1);
        alljnts = angles;
        ...
        for i=1:num_jnts2change
            ...
            for j = 1:numdata
                quat(j,:) = eul2quat(alljnts(j,(i-1)*3+1:(i-1)*3+3),jnts2change_seq{i});
                if quat(j,1) < 0
                    quat(j,:) = quat(j,:)*(-1);
                end
            end
...
%=============================================================================================
function [lengths, minusdLdq,quat_J, GHfvecs] = opensim_get_polyvalues(Mod, angles, quaternions, Mus, iMus, Dofs, GH_seq, SimEn, groundbody,scapulabody)
...
fclose(polyfile);
fclose(logfile);
end %finish Euler approximation and continue with quaternion
