...
% create xml from the osim model to create struct with info
xml_file = strrep(osimfile,'.osim','.xml');
copyfile(osimfile,xml_file);
...
model.serr_ant_wrapping.translation = str2num(osim_struct.Model.BodySet.objects.Body(1).WrapObjectSet.objects.WrapEllipsoid(1).translation);
model.serr_ant_wrapping.dimensions = str2num(osim_struct.Model.BodySet.objects.Body(1).WrapObjectSet.objects.WrapEllipsoid(1).dimensions);

model.lat_dorsi_sup_wrapping.translation = str2num(osim_struct.Model.BodySet.objects.Body(1).WrapObjectSet.objects.WrapEllipsoid(2).translation);
model.lat_dorsi_sup_wrapping.dimensions = str2num(osim_struct.Model.BodySet.objects.Body(1).WrapObjectSet.objects.WrapEllipsoid(2).dimensions);

model.lat_dorsi_mid_wrapping.translation = str2num(osim_struct.Model.BodySet.objects.Body(1).WrapObjectSet.objects.WrapEllipsoid(3).translation);
model.lat_dorsi_mid_wrapping.dimensions = str2num(osim_struct.Model.BodySet.objects.Body(1).WrapObjectSet.objects.WrapEllipsoid(3).dimensions);
...
counter=0;
wrap_info = osim_struct.Model.ForceSet.objects.Schutte1993Muscle_Deprecated;
...
% this is not required for the Autolev and MEX files, so it is optional
    ...
    poly_mus = load([musclepolyfile '_' current_poly]);
    ...
    for imus=1:length(poly_mus.mus_model)
        ...
        for iimus=1:model.nMus
            if strcmp(current_mus.name,model.muscles{iimus}.osim_name) || strcmp(current_mus.name,model.muscles{iimus}.name)
                model.muscles{iimus}.(current_poly).lparam_count = current_mus.num_lparams;
                model.muscles{iimus}.(current_poly).lparams = current_mus.lparams;
                model.muscles{iimus}.(current_poly).lcoefs = current_mus.lcoef;
                allmusi(index) = iimus;
                index=index+1;
                break;
            end
    ...
    if index<model.nMus
        ...
        miss_names = model.muscles{miss_mus(1)}.(current_poly).osim_name;
        for imus=2:length(miss_mus)
            miss_names = [miss_names ', ' model.muscles{miss_mus(imus)}.(current_poly).osim_name];
        end
